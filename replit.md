...
*   **Tekmetric Webhook → Normalized Ingestion (Phase B)**: The Tekmetric webhook receiver dual-writes 3 RO events into the normalized Mongo+Postgres tables via `NormalizedIngestionService`, instrumented with `ingestionVia` attribution. `lib/normalized-ingestion.ts::IngestionOptions.ingestionVia` triggers a private `_stampIngestionVia()` that writes immutable `firstIngestedVia/firstIngestedAt` (set-if-missing via `{$exists:false}` filter) plus always-fresh `lastIngestedVia/lastIngestedAt` to `normalized_work_orders`. Polling (`app/api/cron/tekmetric-sync/route.ts`) passes `"poll"`; webhook helper `runWebhookNormalizedIngestion()` in `app/api/webhooks/tekmetric/route.ts` passes `"webhook"`, called from both terminal and non-terminal branches with vehicle/customer enrichment (cache-first, API fallback, soft-fail on missing VIN). Soak metric exposed at `/api/platform-admin/tekmetric/normalized-ingestion-breakdown?days=N` mirroring the Phase A `index-source-breakdown` endpoint — reports `webhookCoveragePct = webhook/(webhook+poll)` per shop per day. Pre-Phase-B rows surface as `unattributed` rather than disappearing. See TEKMETRIC_5K_SCALING_PLAN.md.

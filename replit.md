...
*   **AutoFlow canonical**: `lib/integrations/autoflow/client.ts` is the single canonical client used by the live `/api/webhooks/autoflow` route and all routes that resolve AutoFlow config or fetch DVIs. The flat `lib/integrations/autoflow.ts`, the re-export shim `lib/integrations/autoflow-legacy.ts`, and the unused stub `lib/autoflow.ts` were all deleted as part of Task #303 — every caller imports from `@/lib/integrations/autoflow` (which resolves to the module's `index.ts`).
...
*   **Tekmetric prewarm**: `lib/integrations/tekmetric/jobs-prewarm.ts` now runs only the bulk shop-level path. The legacy per-RO `/jobs?repairOrderId=…` fallback (and its `TEKMETRIC_BULK_JOBS_PREWARM_ENABLED` env kill-switch consumer in this file) was removed; if the bulk fetch fails, the prewarm records the error and the regular indexing path warms the cache opportunistically. The per-shop helper `isBulkJobsPrewarmEnabledForShop` and the env variable itself remain in `lib/integrations/tekmetric/bulk-jobs.ts` because the backfill chunk pre-pass in `app/api/cron/tekmetric-backfill/route.ts` still reads them.
...
## Integration Modularization (Task #303)
*   **Per-provider directories**: Every integration now lives under `lib/integrations/<provider>/` with a stable file shape: `client.ts` (HTTP client + low-level API), `transform.ts` (raw → domain shape), `sync.ts` (backfill / batch sync), `adapter.ts` (registry adapter implementing `IIntegrationAdapter`), `normalized-adapter.ts` (per-provider implementation of `INormalizedAdapter` for the normalized data layer), `types.ts` (only when there are stable shared types), and `index.ts` (re-exports + adapter registration side-effect).
*   **Protractor**: The 3,950-line `lib/integrations/protractor.ts` monolith and `lib/integrations/protractor-backfill.ts` / `lib/protractor-jobs-prewarm.ts` / `lib/integrations/protractor-legacy.ts` were absorbed into `lib/integrations/protractor/{client,sync,jobs-prewarm,transform,adapter,normalized-adapter,index}.ts`.
*   **Tekmetric**: `lib/tekmetric.ts` (587L superset of API methods — inventory, appointments, work-order/mileage helpers, bulk-jobs windowed fetch) was split into `lib/integrations/tekmetric/api.ts` (unique surface) on top of the existing `client.ts` (`tekmetricRequest`, shop/customer/vehicle/RO basics). All `lib/tekmetric-*.ts` helpers (`auth`, `sync`, `incremental-sync`, `job-index`, `usage-tracker`, `bulk-jobs`, `jobs-prewarm`, `jobs-prewarm-alerter`, `skipped-ro-resolution`, `webhook-subscribe`) moved to `lib/integrations/tekmetric/` with the `tekmetric-` prefix dropped.
*   **AutoFlow**: `lib/integrations/autoflow.ts` (371L superset with snapshot/cache funcs) replaced the slim `lib/integrations/autoflow/client.ts`; the unused stub `lib/autoflow.ts` was deleted.
*   **`normalized-adapters.ts` (1,235L) split**: Shared `INormalizedAdapter` interface, helpers (`generateContentHash`, `generateEntityId`, `createProvenance`, `createSoftDelete`, `parseDate`, `parseNumber`, `cleanString`), and the `getAdapter` / `getSupportedSystems` factory now live in `lib/integrations/core/normalized-adapter.ts`. The `ProtractorAdapter` and `TekmetricAdapter` classes moved to `lib/integrations/<provider>/normalized-adapter.ts`. The factory dictionary `new`s each via per-provider imports.
*   **`normalized-ingestion.ts` (1,458L moved)**: The `NormalizedIngestionService` class is provider-polymorphic (delegates to `getAdapter`) so it moved as-is to `lib/integrations/core/normalized-ingestion.ts` — no per-provider split is meaningful here.
*   **Import surface**: All consumers import from the module root (`@/lib/integrations/<provider>`) which resolves to `index.ts` and re-exports the full surface (`export * from './client'`, `'./api'`, `'./sync'`, `'./transform'`, plus the adapter side-effect `register(...)`). Absolute self-imports inside a module were converted to relative (`./client`, `./api`).

...
*   **Build RO from VHI**: A second one-click button in the Detect Dog extension (next to Pre-fill DVI, gated on the same `dvi_prefill` flag) that proposes customer concerns + jobs for every overdue and due-soon VHI item, then writes them to Tekmetric via the proven two-step job-create pattern. Each created concern/job is stamped with an `[ai-suggested from VHI:<serviceKey>]` marker so re-runs are idempotent. Build attempts are recorded in `admin_audit_logs` under the `build_ro_from_vhi` action for platform-admin observability.
...
*   **Tekmetric Manual Skipped-RO Resolution**: The platform-admin sync-health view exposes a per-RO "Mark resolved" button next to each entry on `recentSkippedRos`. Clicking it POSTs to `/api/admin/sync-health/skipped-ros/resolve` (`{shopId, roId}`), which calls the shared resolver in `lib/tekmetric-skipped-ro-resolution.ts`. The entry is archived into `tekmetric_skipped_ro_archive` with `manualResolution: true` and the actor's email (`resolvedBy`), then removed from the rolling window with `resolvedSkippedRosTotal` incremented and `lastSkippedRosResolvedAt` (and `roSkipsFullyRecoveredAt` when the window empties) stamped. The same helper (`archiveResolvedSkippedRos`) backs the cron's auto-resolve path so both routes share archive-document shape and ordering (archive first, then mutate the progress row). Used for stale ROs whose date window the cursor has already advanced past, where the cron's auto-resolve will never fire.
...
*   **Tekmetric Backfill Chunk-Speed Metrics**: The Tekmetric backfill cron (`/api/cron/tekmetric-backfill`) instruments each chunk with wall-clock duration, RO count, jobs/vehicles/customers cache hit rates (in-memory cache + Mongo cache + work-orders projection all count as hits, only the API fallback is a miss), and 429 backoff time (computed as a delta against a process-wide counter snapshot at chunk start, via `getTekmetric429BackoffMs()` in `lib/integrations/tekmetric/client.ts`; the 429 figure is approximate when multiple shops run concurrently in the same process since one shop's backoff can leak into another's chunk delta). Each cron run persists `lastChunkMetrics` plus a capped 25-entry `recentChunkMetrics` rolling window on `tekmetric_backfill_progress`. The `/api/admin/sync-health` endpoint rolls the window into per-shop median + p95 + max chunk duration and median cache hit %s, and the `/platform-admin/sync-health` page surfaces them as a "Chunk speed (Tekmetric)" table sorted slowest-p95 first, with a "slow" badge for any shop whose p95 exceeds 10 minutes.
...
*   **Tekmetric Jobs-Cache Pre-Warm Alerting**: When the onboarding pre-warm in `lib/tekmetric-jobs-prewarm.ts` finishes with `errors > 0` (a `/jobs` listing or fetch failed) or `capped: true` (the 90-day terminal-RO count hit the 500-RO pre-warm cap), `lib/tekmetric-jobs-prewarm-alerter.ts` emails all platform admins with the shopId/tekmetricShopId, completedAt, error count, capped flag, RO/job counts, and a link to `/platform-admin/sync-health`. Dedup is per-shop in `tekmetric_jobs_cache_prewarm_alerts` keyed on the anomaly state (`errors`, `capped`, or `errors,capped`) so re-warming via the platform-admin UI on a shop in the same state is suppressed and on-call is only re-paged when a new failure mode appears. Alert emission is wrapped in try/catch inside the prewarm function so a Resend or Mongo hiccup can never break the onboarding flow.
...
## Build & Test Pipeline
*   **Plan-build smoke tests** run automatically on every build via the `prebuild` npm hook. The umbrella script `npm run test:plan-build` invokes both `test:plan-build-task-163` and `test:plan-build-task-165`. Any new plan-build smoke test should be appended to the umbrella script so it runs on every Vercel/Render deploy. A failing smoke test exits non-zero and blocks the build.

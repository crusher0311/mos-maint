...
*   **Tekmetric Backfill Drain Worker**: A standalone Node script (`scripts/drain-tekmetric-backfill.ts`, run via `npm run drain:tekmetric-backfill`) that imports `backfillShopChunk` from the cron route and walks every incomplete Tekmetric shop to completion in one long-running process — bypasses the cron's per-tick chunk budget and 300s ceiling. Holds an exclusive Mongo lease (`tekmetric_drain_lock`, 5-min TTL with 60s refresh) so cron GET/POST handlers no-op while it runs, eliminating the cursor-clobber race. Configurable via `DRAIN_PARALLELISM`, `DRAIN_MAX_CHUNKS_PER_SHOP`, `DRAIN_HEARTBEAT_MS`, `DRAIN_SHOP_IDS`. Loads `server-only`-tainted modules through a require-hook stub at `scripts/_stubs/server-only-stub.cjs`.

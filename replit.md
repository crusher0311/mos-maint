...
*   **Rescue Rover AI Voice Agent**: An AI-powered phone assistant handles inbound calls using Twilio media streams, Deepgram for STT/TTS (with automotive keyword boosting), and OpenAI GPT-4o for conversational intelligence. Features include: shop context injection (name, address, hours, services), customer/vehicle lookup with VHI data, barge-in detection (caller can interrupt AI), post-call AI summary generation via GPT-4o-mini, silence detection with caller prompting, tools for business hours and shop info queries, and comprehensive call logging with cost tracking.

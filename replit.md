...
*   **VHI API Endpoint**: Two versions: `GET /api/vehicles/[vin]/vhi` (session-authenticated, internal) and `GET /api/external/vehicles/[vin]/vhi` (API-key-authenticated, external). Both return Vehicle Health Indicator data as JSON — health score (0–100 with tier label), vehicle info, and bucketed maintenance items (overdue/dueSoon/upcoming). Shared scoring logic in `lib/vhi-score.ts`. Reads from cached plan data.

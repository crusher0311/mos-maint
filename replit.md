...
*   **Data Management**: Data is transitioning from MongoDB Atlas to PostgreSQL. Plan caching stores assembled plan buckets for instant loads with mileage tolerance-based invalidation. Incremental plan updates recalculate milesToGo/daysToGo without full rebuilds (~50ms vs full rebuild).
*   **Backfill & Sync Optimizations**: Hot-start prioritizes last 30 days for new shops (usable in minutes vs days). Parallel shop processing (3 concurrent) with PostgreSQL advisory locks. Webhook-driven real-time sync updates normalized tables immediately. Priority-based prefetch queue prioritizes in-progress ROs, webhook updates, and recent views.
*   **Integration Mechanisms**: A modular integration layer supports shop management systems (e.g., Tekmetric, Protractor) through `IIntegrationAdapter` and `IntegrationFacade` patterns, incorporating webhooks, incremental sync, OAuth, and rate limiting. An `ISMSAdapter` interface normalizes SMS data. The dashboard uses `lib/dashboard-data-service.ts` as a facade to fetch data from all integration types (Tekmetric, Protractor, AutoFlow) through a unified interface.

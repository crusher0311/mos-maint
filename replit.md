...
*   **Enhance Notes (AI Findings)**: A purple "Enhance Notes" button in the Detect Dog extension that rewrites technician inspection findings into professional customer-facing language using AI. Reads existing findings from the active inspection, sends them to `/api/extension/enhance-findings` (OpenAI GPT-4o-mini), and writes the enhanced text back to Tekmetric. Preserves measurements and technical accuracy while improving grammar and clarity.

...
*   **Production Log Caching**: Production logs from Better Stack (ClickHouse) are synced to a Supabase `production_logs` table with 30-day retention. A cron endpoint (`/api/cron/log-sync`) fetches recent logs in batches and deduplicates via content hash. The `/logs` page reads from Supabase for fast, filterable access with search, level filtering, and time ranges up to 30 days. Platform admin auth required.

...
**Production deploys (Render, mos-tools): Brandon pushes, Brandon deploys.** Commit work locally on main but DO NOT `git push` — pushing to GitHub main is what triggers the Render auto-deploy, and that trigger belongs to Brandon. When work is ready: commit, then tell Brandon "ready to push" with a summary of what the push will deploy. Never trigger Render deploys or rollbacks via the Render API on my own either. If prod is actively down/degraded, ask Brandon first and act only with his go-ahead. Note: after any Render rollback, push-to-main auto-deploy may not fire — check that Render actually starts a build after the next push. (Set 2026-08-06 after I pushed/rolled back/re-deployed unilaterally during the Shopmonkey Mongo-saturation incident.)

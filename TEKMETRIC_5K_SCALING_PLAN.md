...
**Effort:** 1 week
**Risk:** Low
**Status:** Complete (3a + 3b shipped fully; 3c framework shipped, awaiting Tekmetric subscription API spec to enable; 3d webhook-latency hardening + receipt-drop alerting shipped under task #376).
...
- **3d. Webhook latency hardening + receipt-rate alerting (DONE — task #376)**: After the Phase B NIS dual-write + #360 service-jobs/line-items expansion landed inline, p95 webhook handler latency drifted up. We now (1) defer the heavy work — vehicle/customer enrichment, NIS dual-writes, VHI rebuild trigger — off the request thread via a `__deps.defer` seam (default `setImmediate`, fire-and-forget with error logging) so the inline handler returns in the low double-digit ms range; (2) persist `handlerDurationMs` on every `tekmetric_webhook_logs` row so latency is observable without grepping logs; (3) surface aggregate p50/p95/p99 over 24h + 7d on `/api/platform-admin/tekmetric/webhook-subscription-status`, alongside `incrementalSync.lastRunAt` for the polling safety net (read from the `mos`-db `cron_runs` collection), and rendered by a new admin UI page at `/platform-admin/tekmetric-webhook-health` (linked from the platform-admin sidebar); (4) extend `tekmetric-webhook-health` to fire when last-hour p95 > `TEKMETRIC_WEBHOOK_P95_LATENCY_MS` (default 3000ms, 30-sample minimum) or when a per-shop 24h receipt count drops below `TEKMETRIC_WEBHOOK_RECEIPT_DROP_RATIO` × (7d/7) with a 7d-volume floor of 14 to suppress noise; (5) bump the `tekmetric-webhook-health` cron from daily 09:00 UTC to hourly at `:07` so the 1-hour p95 evaluation window is actually re-run hourly (worst-case detection ~2h instead of ~24h), with per-(shop,UTC-day) dedup on `tekmetric_webhook_health_alerts` keeping the silent / drop alerts at most once-per-shop-per-day. All three alert kinds (silent / drop / latency) consolidate into a single email per cron run. The Step 1 evidence script — `scripts/tekmetric-webhook-latency-measure.ts` — reports per-shop receipt counts in 24h vs 7d-daily-avg vs 30d-daily-avg plus latency percentiles over the same windows, and writes a JSON dump under `scripts/output/`. Smoke coverage: `tests/tekmetric-webhook-latency.smoke.ts` asserts <500ms inline + deferred work queued via `__deps.defer` for both terminal and non-terminal RO events.

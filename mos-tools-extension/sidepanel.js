...
// Reveal platform-admin-only UI (Detect Dog: Shop Migration Wizard tab)
...
const RO_INDEPENDENT_TABS = ['rates', 'concern', 'migrate'];
...
// /api/extension/tekmetric-migration/* orchestrator (T002/T004).
...
const MIG = {
  ...
  sourceShopId: null,
  destShopId: null,
  // Latest server responses, kept client-side because the orchestrator
  // does not persist plan/preview between calls (only the final mapping).
  lastDump: null,           // { counts, preview, errors }
  lastCorePlan: null,       // { rosWillCreate, needsOverride[], preview[], ... }
  lastCoreResult: null,     // { counts, mapping, failures }
  lastExtrasResult: null,   // { successes[], failures[] }
};
...
function migPopulateShopSelects() {
  ...
  if (!shops.length) {
    ...
    if (hint) hint.textContent = 'No Tekmetric shops are linked to your account. Ask an admin to add one before starting a migration.';
  }
}
  ...
  try {
    ...
    wrap.querySelectorAll('.migrate-resume-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        // Show all steps so the operator can re-run any phase. The "needs
        // override" sub-list will only have content after a load-core
        // dry-run is re-run from the resumed state.
        migShowStep(5);
        migRefreshAudit();
      });
    });
...
function migRenderDumpSummary(payload) {
  // dump POST returns { ok, counts, errors, preview[] } where preview is
  // the FULL list of ROs (not truncated) — we render it inside a
  // scrollable table so operators can review every row.
...
function migRenderLoadCorePlan(plan, isDryRun) {
  // planLoadCore returns:
  //   rosInDump, rosWithDumpError, rosAlreadyMigrated, rosWillCreate,
  //   jobsWillCreate, needsOverride[], preview[{sourceRo, customer,
  //   vehicle, jobs, status: 'will-create'|'already-migrated'|
  //   'needs-override'|'dump-error'}]
...
function migRenderLoadCoreResult(res) {
  // executeLoadCore returns { counts:{successes,failures,
  // reusedAlreadyMigrated}, mapping[], failures[{sourceRo, sourceRoId,
  // step, status, error}] }
  const c = res.counts || {};
...
function migRenderExtrasResult(res) {
  // executeLoadExtras returns { successes:[{roId, inspectionsCreated, photosCreated, photosFailed}], failures[] }
...
function initMigrateWizard() {
  ...
  // When the migrate tab is opened, refresh tokens + recent runs.
  document.querySelector('[data-tab="migrate"]')?.addEventListener('click', () => {
    migPopulateShopSelects();
    migRefreshTokenStatus();
    migLoadExistingRuns();
    if (MIG.runId) migRefreshAudit();
  });
}

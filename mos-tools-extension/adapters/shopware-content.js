...
try {
  globalThis.MosTelemetryCore?.installErrorHooks({
    ...
    // Shop-scope the error throttle across in-tab tenant switches.
  ...
  try {
    ...
    if (result && result.config) {
      ...
      if (result.config.intervals) {
        ...
        // Task #439: respect optional per-shop `label` overrides and `hidden`
        // flag on each bucket. Hidden buckets are skipped entirely.

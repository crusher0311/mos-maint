const CRON_JOBS = [
  {
    name: "autoflow-dashboard-notifications",
    path: "/api/cron/autoflow-dashboard-notifications",
    schedule: "* * * * *",
    lockTtlMs: 55 * 1000,
    timeoutMs: 30 * 1000,
    method: "GET",
    description: "Retry only durable AutoFlow dashboard invalidations (bounded, no upstream work)",
  },
  {
    name: "protractor-callback-drain",
    path: "/api/cron/protractor-callback-drain",
    schedule: "* * * * *",
    lockTtlMs: 55 * 1000,
    timeoutMs: 50 * 1000,
    method: "GET",
    description: "Drain durable Protractor callbacks (callback-only, frequent)",
  },
  {
    name: "report-runs",
    path: "/api/cron/report-runs",
    schedule: "* * * * *",
    method: "GET",
    timeoutMs: 5 * 60 * 1000,
    description: "Process one durable report build and refresh active saved dashboards",
  },
  {

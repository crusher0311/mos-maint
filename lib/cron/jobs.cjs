const CRON_JOBS = [
  ...
  {
    // Tier 1 webhook-outage fallback. Tails AppFueled's public Protractor
    // request log every minute and replays events for the ~10 shops we share
    // with AF through our own /api/callbacks/protractor handler. Same code
    // path as a real webhook would hit — see app/api/cron/protractor-af-log-tail/
    // route.ts for full rationale, design, and removal criteria.
    //
    // Cadence: every 1 min — node-cron's tightest standard granularity. Gives
    // ~60s freshness for AF-shared shops vs the near-instant a real webhook
    // would give. Handler is fast on quiet ticks (1 GET to AF + 0 dispatches)
    // and naturally bounded by MAX_EVENTS_PER_TICK=200 + the 5-RPS Protractor
    // cap on the dispatched callback fetches.
    //
    // ROLLBACK: delete this entry when Protractor webhook delivery resumes.
    name: "protractor-af-log-tail",
    path: "/api/cron/protractor-af-log-tail",
    schedule: "* * * * *",
    method: "GET",
    lockTtlMs: 2 * 60 * 1000,
    timeoutMs: 90 * 1000,
    description: "Tail AF's Protractor callback log (every 1 min — webhook-outage fallback)",
  },
  {
    // Tier 2 webhook-outage fallback. Polls the active work-order list for
    // every Protractor shop EXCEPT the 10 AF-shared shops (default-excluded
    // in the handler — Tier 1 covers them via AF's log at ~60s freshness).
    // For the remaining ~17 shops, every 3 min, diff the active set against
    // a per-shop Mongo snapshot and synthesize callback replays for state
    // transitions (disappeared, transitioned-into-invoiced, newly opened).
    //
    // ONE list call per shop per tick — no per-WO detail fetches, no
    // ingestion, no pregeneration. The downstream callback handler does
    // the per-WO work asynchronously (its existing background-enrich path)
    // exactly as it would for a real webhook. See route header for the
    // full design, kill-switch env flag, AF-shared exclude-list override
    // (PROTRACTOR_STAGE_REFRESH_EXCLUDE_SHOPS), and removal criteria.
    //
    // AF-shared overlap was originally going to rely on the callback
    // handler's de-dup window, but the GET dedup is only 60s AND keyed on
    // `operation` (Tier 1 emits real ops like Update/Delete, Tier 2 always
    // emits UPDATE) — so overlap would double-bill the Protractor API on
    // those shops. Hence the default exclude-list.
    //
    // KILL SWITCH: set PROTRACTOR_STAGE_REFRESH_DISABLED=true on the
    // Render service to disable this cron at runtime without redeploying.
    // Tier 1 stays active either way.
    //
    // ROLLBACK: delete this entry when Protractor webhook delivery resumes.
    name: "protractor-stage-refresh",
    path: "/api/cron/protractor-stage-refresh",
    // Cadence */3 (3 min) chosen after first smoke at */2 ran ~125s with
    // pLimit(4): Protractor's WO list endpoint is ~4-5s per 100-WO page,
    // and 27 shops × 1-2 pages each leaves zero headroom inside 120s.
    // Handler now uses pLimit(8) shop concurrency; 3-min cadence with
    // 4-min lock TTL keeps overlapping ticks impossible even on a slow
    // Protractor day, and 3-min freshness is still 10× better than the
    // 30-min protractor-sync baseline.
    schedule: "*/3 * * * *",
    method: "GET",
    lockTtlMs: 4 * 60 * 1000,
    timeoutMs: 170 * 1000,
    description: "Poll Protractor active WO lists and replay stage transitions (every 3 min — webhook-outage fallback)",
  },
  {
    name: "shopware-sync",
  ...
  },
  {
    ...
    // mos-maint-background-v2) at 10:00pm Central, Mon-Fri. Because there is
    // NO pause on Sat/Sun, the Friday 10pm resume carries the workers through
    ...
    method: "GET",
    description: "Resume background workers (10:00pm Central Mon-Fri; carries through the weekend)",
  },
  {
    ...
    // symptoms). No weekend pause: the Friday 10pm resume runs uninterrupted

const CRON_JOBS = [
  ...
  {
    // Weekday off-hours boost — task #447. Diagnosis #443
    ...
    // ~every 3 days. This entry mirrors the weekend boost cadence
    // (every 15 min at :00/:15/:30/:45) but is gated to Tue–Fri 02:00–11:00
    // UTC — roughly 21:00–06:00 US Eastern, when US auto shops are mostly
    // closed and the Tekmetric quota is quiet. Auto-stops at 12:00 UTC
    // (~07:00 ET) before US shops open and dashboard/VHI traffic ramps.
    //
    ...
    // process), per-shop in-flight locks via `inflight-lock.ts`.
    //
    ...
    // daily tick (which is 01:00 UTC, before this entry's window starts).
    name: "weekday-backfill-boost",
    path: "/api/cron/tekmetric-backfill",
    schedule: "0,15,30,45 2-11 * * 2-5",
    method: "GET",
    lockTtlMs: 60 * 60 * 1000,
    timeoutMs: 25 * 60 * 1000,
    description: "Tekmetric backfill weekday off-hours boost (every 15 min, Tue-Fri 02:00-11:45 UTC)",
  },
  {

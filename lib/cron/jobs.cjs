const CRON_JOBS = [
  ...
  },
  {
    ...
    path: "/api/cron/tekmetric-backfill",
    // Bumped from hourly (:30) to every 30 min (:00 and :30) on Sat+Sun.
    // Chunk wall-clock is <10 min after the jobs-cache speedup, so the
    // 30-min cadence still leaves a 3x safety margin. Stays staggered
    // 10 min off Protractor (:10/:40) and 20 min off Shop-Ware (:20/:50)
    // so the three providers never co-fire in the same minute.
    schedule: "0,30 * * * 6,0",
    method: "GET",
    lockTtlMs: 60 * 60 * 1000,
    timeoutMs: 25 * 60 * 1000,
  ...
  },
  {
    ...
    path: "/api/cron/protractor-backfill",
    // Bumped from hourly (:40) to every 30 min (:10 and :40) on Sat+Sun
    // to mirror the Tekmetric weekend boost. Stays staggered 10 min off
    // Tekmetric (:00/:30) and 10 min off Shop-Ware (:20/:50) so the
    // three providers never co-fire in the same minute.
    schedule: "10,40 * * * 6,0",
    method: "GET",
    lockTtlMs: 60 * 60 * 1000,
    timeoutMs: 25 * 60 * 1000,
  ...
  },
  {
    ...
    path: "/api/cron/shopware-backfill",
    // Bumped from hourly (:50) to every 30 min (:20 and :50) on Sat+Sun
    // to mirror the Tekmetric weekend boost. Stays staggered 20 min off
    // Tekmetric (:00/:30) and 10 min off Protractor (:10/:40) so the
    // three providers never co-fire in the same minute.
    schedule: "20,50 * * * 6,0",
    method: "GET",
    lockTtlMs: 60 * 60 * 1000,

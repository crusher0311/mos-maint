...
export async function buildPartnerVhiResponse(
  req: NextRequest,
  { shopId, isPartner, partnerId, requestId }: ExternalApiContext,
  overrides?: PartnerVhiOverrides,
) {
    const pathParts = req.nextUrl.pathname.split("/");
    const vinIndex = pathParts.indexOf("vehicles") + 1;
    const vin = overrides?.vin.toUpperCase() ?? pathParts[vinIndex]?.toUpperCase();
    const requestedMode = overrides?.mode ?? req.nextUrl.searchParams.get("mode");
    ...
    let resolvedShopId = overrides?.shopId ?? shopId;

    if (isPartner && !overrides) {

...
const CALLBACK_TRIAL_ENABLED_ENV = "PROTRACTOR_CALLBACK_TRIAL_ENABLED";
const DEVELOPMENT_RELAY_APPROVAL_ENV = "PROTRACTOR_DEVELOPMENT_RELAY_APPROVED";
const DEVELOPMENT_ENVIRONMENT_VALUES = new Set(["development", "dev", "preview", "staging", "qa"]);
...
/**
 * Keep provenance deliberately coarse.  In particular, do not put domains,
 * deployment ids, hostnames, or other operator/environment values in usage
 * records: this value is only for reconciling production vs preview vs local
 * traffic.
 */
function resolveProtractorEnvironment(env) {
  // A Replit development domain identifies a preview even when the preview
  // runs a production build with NODE_ENV=production.  Treat that deployment
  // signal as authoritative so production build defaults cannot bypass the
  // explicit development relay approval gate.
  if (
    typeof env.REPLIT_DEV_DOMAIN === "string" &&
    env.REPLIT_DEV_DOMAIN.trim() !== ""
  ) {
    return "development";
  }
  const raw = [
    env.PROTRACTOR_ENVIRONMENT,
    env.MOS_ENVIRONMENT,
    env.APP_ENV,
    env.NODE_ENV,
  ...
  if (DEVELOPMENT_ENVIRONMENT_VALUES.has(normalized)) {
    return "development";
  }
  if (normalized === "test") return "test";
  if (normalized) return "unknown";
  return env.RENDER ? "production" : "unknown";
}

function isExplicitDevelopmentRelayApproval(env) {
  return [
    env[DEVELOPMENT_RELAY_APPROVAL_ENV],
    env.PROTRACTOR_DEVELOPMENT_APPROVED,
    env.PROTRACTOR_RELAY_DEVELOPMENT_APPROVED,
    env.PROTRACTOR_DEV_RELAY_APPROVED,
    env.PROTRACTOR_RELAY_APPROVED,
  ].some(value => value === "true");
}

function isDevelopmentEnvironment(env) {
  return resolveProtractorEnvironment(env) === "development";
}
...
function evaluateProtractorOutboundPolicy(env, nowMs = Date.now()) {
  ...
  // A local/preview process must never send Protractor traffic directly.  The
  // approval is intentionally separate from relay mode so copying a relay
  // URL/secret into a development process is not, by itself, authorization to
  // exercise the provider.
  if (
    isDevelopmentEnvironment(env) &&
    (
      env.PROTRACTOR_RELAY_MODE !== "relay" ||
      !isExplicitDevelopmentRelayApproval(env)
    )
  ) {
    return {
      allowed: false,
      reason: "development_relay_required",
      identity: resolveInstanceIdentity(env),
    };
  }

  let callbackOnly = false;
  let requireTimedTrial = false;
  // Interactive admission is never granted by local environment flags.  The
  // effective policy can turn this on only after validating the persisted
  // Mongo activation scope.
  let allowInteractive = false;
  let callbackNotBeforeMs = null;
  const rawTrialEnabled = env[CALLBACK_TRIAL_ENABLED_ENV];
  const rawCanaryUntil = env[CALLBACK_CANARY_UNTIL_ENV];
  const rawReplayNotBefore = env[CALLBACK_REPLAY_NOT_BEFORE_ENV];
  if (rawTrialEnabled != null && rawTrialEnabled !== "true" && rawTrialEnabled !== "false") {
    return {
      allowed: false,
      reason: "malformed_callback_trial_flag",
      identity: resolveInstanceIdentity(env),
    };
  }
  if (rawTrialEnabled === "true") {
    // The activation-started trial is deliberately a distinct policy mode.
    // Combining it with the old wall-clock canary would make the local
    // process disagree with the Mongo admission record.
    if (rawCanaryUntil != null) {
      return {
        allowed: false,
        reason: "conflicting_callback_trial_policy",
        identity: resolveInstanceIdentity(env),
      };
    }
    callbackOnly = true;
    requireTimedTrial = true;
  } else if (rawCanaryUntil != null) {
...
module.exports = {
  ...
  CALLBACK_TRIAL_ENABLED_ENV,
  CALLBACK_REPLAY_NOT_BEFORE_ENV,
  DEVELOPMENT_RELAY_APPROVAL_ENV,
  DENY_ENV,

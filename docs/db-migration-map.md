...
**Final decommission status (2026-08-01, task #1001): STILL BLOCKED — aborted at Step 1 again.** The re-audit found **623** live files holding a Mongo handle (335 `app/`, 183 `lib/`, 96 `scripts/`, 9 `tests/` — up from 487, because the intervening cutover work added flag-gated dual paths that default Mongo-canonical). No `*_PG_CANONICAL` flag is set in any environment; all `WRITE_MONGO_*` shadow writes are still on; prerequisite tasks (#560 job_index reader move, #327 direct-DB list cleanup, operator flips + soaks) are unmerged. Nothing was removed. Audit: `docs/audits/2026-08-01-task-1001-decommission-aborted.md`. The operator checklist for the eventual decommission (code removal, env cleanup, snapshot → pre-drop counts → drops → Atlas pause ≥30d → delete, docs closeout) is now pre-staged at `docs/runbooks/mongo-final-decommission-checklist.md`.

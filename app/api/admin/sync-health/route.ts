...
export async function GET() {
  try {
    ...
    // Stuck-shop diagnostics for Tekmetric backfill. A shop is "stuck" if:
    ...
    const tekmetricDiagnostics = tekmetricBackfillProgress
      ...
      .map((p: any) => {
        ...
        // pre-existing rows that don't have this field yet, fall back to
        // lastRunAt (best available signal until they next run).
        const cursorMoveMs = p.lastCursorMoveAt
          ? new Date(p.lastCursorMoveAt).getTime()
          : lastRunMs;
        const daysCursorFrozen = cursorMoveMs == null ? null : (now - cursorMoveMs) / (24 * 60 * 60 * 1000);

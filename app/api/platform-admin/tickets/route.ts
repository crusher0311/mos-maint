...
import {
  aggregateSupportTickets,
  countSupportTickets,
  findOneAndUpdateSupportTicketById,
  insertSupportTicket,
  listSupportTickets,
  updateManySupportTickets,
} from "@/lib/data/repositories/support-tickets";
import { listShopsByLegacyIds } from "@/lib/data/repositories/shops";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const autoCloseResult = await updateManySupportTickets(
      {
        status: "resolved",
    ...
    const [rawTickets, totalCount] = await Promise.all([
      listSupportTickets(query, { sort: { createdAt: -1 }, skip, limit }),
      countSupportTickets(query),
    ]);

    const shopIds = rawTickets
      .filter((t) => t.shopId && !t.shopName)
      .map((t) => Number(t.shopId))
      .filter((n) => Number.isFinite(n));

    let shopMap: Record<number, { name: string; locationIdentifier?: string }> = {};
    if (shopIds.length > 0) {
      // NOTE: tickets store shopId, but legacy shop docs here are keyed
      // by the alternate `id` field; keep that exact behavior.
      const shops = await listShopsByLegacyIds(shopIds, {
        id: 1,
        name: 1,
    ...
    const tickets = rawTickets.map((ticket) => {
      const sid = ticket.shopId != null ? Number(ticket.shopId) : null;
      const fallback = sid != null ? shopMap[sid] : undefined;
      return {
        ...ticket,
        shopName: ticket.shopName || fallback?.name || null,
        locationIdentifier:
          ticket.locationIdentifier || fallback?.locationIdentifier || null,
      };
    });

    const stats = await aggregateSupportTickets<{ _id: string; count: number }>([
      {
        $group: {
          _id: "$status",
          count: { $sum: 1 },
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const result = await findOneAndUpdateSupportTicketById(ticketId, updateOps);
    ...
    const statusLabel = statusLabels[ticketStatus] || ticketStatus;
    const ticketShopIdNum =
      ticket.shopId != null && Number.isFinite(Number(ticket.shopId))
        ? Number(ticket.shopId)
        : undefined;
    ...
    if (ticket.userEmail && (status || message)) {
      ...
      try {
        // shop_users lookup is intentionally kept here — multi-collection
        // join with shop_users not yet migrated.

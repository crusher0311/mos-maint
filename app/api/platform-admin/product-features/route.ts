...
export async function GET(req: NextRequest) {
  try {
    ...
    let features: any[] = [];
    if (productId) features = await productFeatureRepo.listByProduct(productId);
    else if (planId) features = await productFeatureRepo.listByPlan(planId);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const feature = await productFeatureRepo.create(body);
...
export async function PUT(req: NextRequest) {
  try {
    ...
    const { id, ...data } = body;
    const feature = await productFeatureRepo.update(id, data);
    return NextResponse.json({ ok: true, feature });
  } catch (e: any) {
    if (e?.message?.includes("Unauthorized")) return NextResponse.json({ ok: false, error: "Unauthorized" }, { status: 401 });
    return NextResponse.json({ ok: false, error: e.message }, { status: 500 });
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    await productFeatureRepo.delete(id);

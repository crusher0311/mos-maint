...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const promoCodes = await promoCodeRepo.list({ search, includeArchived });
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (e?.message?.includes("Unauthorized")) return NextResponse.json({ ok: false, error: "Unauthorized" }, { status: 401 });
    if (e?.message?.includes("unique")) return NextResponse.json({ ok: false, error: "Promo code already exists" }, { status: 409 });
    return NextResponse.json({ ok: false, error: e.message }, { status: 500 });
  }
}
...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    const { id, ...data } = body;
    const promoCode = await promoCodeRepo.update(id, data);
    return NextResponse.json({ ok: true, promoCode });
  } catch (e: any) {
    if (e?.message?.includes("Unauthorized")) return NextResponse.json({ ok: false, error: "Unauthorized" }, { status: 401 });
    return NextResponse.json({ ok: false, error: e.message }, { status: 500 });
...
export async function DELETE(req: NextRequest) {
  ...
  try {
    ...
    await promoCodeRepo.archive(id);

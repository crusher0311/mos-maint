...
import { productRepo } from "@/lib/db/repositories/sales-marketing";
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const products = await productRepo.list({ search, category, includeArchived });
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!body.name || !body.slug) return NextResponse.json({ ok: false, error: "Name and slug are required" }, { status: 400 });

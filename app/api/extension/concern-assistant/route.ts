  ...
  try {
    ...
    if (action === "followup") {
      ...
      const cachedQuestions = await getCachedFollowupQuestions(concern).catch((err) => {
        console.warn("[Concern Assistant] followup cache read failed (non-fatal):", err?.message);
        return null;
      });
      ...
      if (cachedQuestions !== null) {
        ...
        console.log(`[Concern Assistant] followup cache HIT (${questions.length} questions after skip+prefer filter)`);
      } else {
        ...
        await setCachedFollowupQuestions(concern, rawQuestions).catch((err) => {
          console.warn("[Concern Assistant] followup cache write failed (non-fatal):", err?.message);
        });

        // Apply this shop's skip hints as post-processing (avoid drop + prefer reorder).
        const hints = await getSkipHints({ db, mosShopId, symptomCategory });
        questions = applySkipHints(rawQuestions, hints);
        console.log(`[Concern Assistant] followup cache MISS — cached ${rawQuestions.length} questions, returning ${questions.length} after skip+prefer filter`);
      }

...
function ShopWareSection({
  ...
  async function handleSave() {
    ...
    setMessage(null);
    try {
      const effectiveTenantId = tenantId ? parseInt(tenantId) : parseInt(swShopId);
      const res = await fetch("/api/settings/shopware", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ tenantId: effectiveTenantId, swShopId: parseInt(swShopId) }),
      });
      const data = await res.json();
      if (res.ok) {
    ...
    <div className="space-y-3 border-t pt-4">
      <div>
        ...
        <input
          ...
          placeholder="e.g. 3"
        />
        <p className="text-xs text-gray-500 mt-1">Your Shop-Ware shop ID</p>
      </div>
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">Tenant ID <span className="text-gray-400 font-normal">(optional)</span></label>
        <input
          type="text"
          value={tenantId}
          ...
          placeholder="e.g. 42"
        />
        <p className="text-xs text-gray-500 mt-1">Only needed for multi-location accounts. Single-location accounts can leave this blank — we&apos;ll use your Shop ID as the tenant ID.</p>
      </div>

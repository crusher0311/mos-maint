...
- **Per-user browser bearer works against the public API — the extension bootstrap proof.** The app.shopmonkey.cloud SPA sends a per-user `Authorization: Bearer` on its own API calls; that bearer is accepted by `GET /v3/user/logged-in` (documented current-user endpoint) and `GET /v3/location`, giving a server-probeable current-session subject + shop membership without touching the stored shop-level API key. Capture needs webRequest `extraHeaders` (Authorization on CORS'd calls). Contrast: AutoFlow (v3 PHP/v4 Laravel) is cookie-only with NO confirmed identity endpoint, and Shop-Ware has none — both stay calmly unsupported for bootstrap.

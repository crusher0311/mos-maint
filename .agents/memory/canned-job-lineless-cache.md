...
**Root cause of "fresh enrichment still lineless" (2026-07-23):** the manual sync route (`app/api/protractor/sync/route.ts`) had a THIRD copy of the detail-fetch logic that still called `fetchServicePackageTemplateDetail` (`/ServicePackageTemplate/Read/{id}`) unconditionally — 404s for every template on v1.0/template-fallback shops (shop 219: 3,024 consecutive cached 404s), so every item fell back to the titles-only list summary. And `classifySyncCannedJobsBatchSource` stamped it "enriched" anyway, because v1.0 LIST summaries already carry `ServicePackageHeader` — header presence alone is NOT proof of enrichment. Fixed: sync route now dispatches on `listResult.source` like `enrichCannedJobsWithDetails`, and the classifier downgrades any lineless batch to "sync-partial". Diagnosis tell: `protractor_template_<shop>_*` cache docs all `is404:true` with timestamps matching the cache write = wrong-endpoint sync run.
